...
exports.bulkCreate = async (req, res) => {
  try {
    ...
    const { product_id, supplier_id, notes, variants } = req.body;

    if (!product_id || !supplier_id) {
      return fail(res, 400, "VALIDATION_REQUIRED_FIELDS", "product_id and supplier_id are required");
    }
    ...
    return res.status(201).json({ success: true, data, count: data.length });
  } catch (err) {
