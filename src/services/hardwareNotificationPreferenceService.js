...
class HardwareNotificationPreferenceService {
  /**
  ...
  async getPreference(owner_id, type) {
    const lastPref = await prisma.hardwareNotification.findFirst({
      where: {
        owner_id,
        type,
      },
  ...
  async setPreference(owner_id, type, is_enabled) {
    ...
    const preference = await prisma.hardwareNotification.create({
      data: {
        owner_id,
        type,
...
export default new HardwareNotificationPreferenceService();
