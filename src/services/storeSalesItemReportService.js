...
class StoreSalesItemReportService {
  async salesByItem(
    ...
    // SUMMARY
    //
    // Sales = original selling price × original quantity sold.
    // Discounts are now included in the calculation.
    // ============================================================
    ...
    if (sections.has("categories")) {
      addTask(
        ...
        prisma.$queryRaw`
          ...
          HAVING
            SUM(
              COALESCE(ssi.sp, 0) * COALESCE(ssi.qty, 0)
            ) > 0
    ...
    // RETURNS
    //
    // Returns are reported separately and never deducted from Sales.
    // ============================================================
    if (sections.has("returns")) {
      addTask(
        "returnRows",
        prisma.$queryRaw`
          SELECT
            COALESCE(
              SUM(COALESCE(scri.qty, 0)),
              0
            )::int AS returned_units,

            COALESCE(
              SUM(COALESCE(scri.amount, 0)),
              0
      ...
      addTask(
        ...
        prisma.$queryRaw`
          SELECT
            ...
            COALESCE(
              SUM(COALESCE(sl.qty_remaining, 0)),
              0
    ...
    for (let index = 0; index < taskNames.length; index += 1) {
      loaded[taskNames[index]] = results[index];
    }

    const summaryRows = loaded.summaryRows ?? [];
    const categoryRows = loaded.categoryRows ?? [];
    const productRows = loaded.productRows ?? [];
    const supplierRows = loaded.supplierRows ?? [];
    const trendRows = loaded.trendRows ?? [];
    const returnRows = loaded.returnRows ?? [];
    const lowStockRows = loaded.lowStockRows ?? [];
    ...
    const categories = categoryRows.map((category) => {
      const categorySales = Number(
        category.total_sales ?? 0,
      );

      const categoryUnits = Number(
    ...
    const topProducts = productRows.map((product) => {
      ...
      const productCogs = Number(
        product.total_cogs ?? 0,
      );

      const productUnits = Number(
        product.total_units ?? 0,
      );

      const productProfit =
        productSales - productCogs;

      const productMargin =
        productSales > 0
          ? Number(
              (
                (productProfit / productSales) *
                100
              ).toFixed(1),
            )
          : 0;
    ...
    if (sections.has("summary")) {
      response.summary = {
        // Sales = selling price × original quantity sold (before discount).
        total_item_sales: Number(
          totalItemSales.toFixed(2),
        ),
        ...
        // Original sold-item COGS.
        total_cogs: Number(
          totalItemCogs.toFixed(2),
        ),

        // Gross item profit (actual revenue - COGS).
        profit: Number(
          grossItemProfit.toFixed(2),
        ),
