...
class StoreProductService {
  ...
  async list(owner_id) {
    try {
      ...
      SELECT 
        ...
        CASE 
          WHEN p.category_id IS NOT NULL THEN json_build_object(
        ...
        CASE 
          WHEN p.unit_id IS NOT NULL THEN json_build_object(
            'unit_id', u.unit_id,
    ...
    `;
      ...
      return rows.map((p) => {
        ...
        return {
          ...
          cp: p.cp !== null ? Number(p.cp) : null,
          sp: p.sp !== null ? Number(p.sp) : null,
  ...
  async getById(owner_id, product_id) {
          ...
          (
            SELECT json_agg(
              json_build_object(
                ...
                'supplier',
                CASE
                  WHEN sl.supplier_id IS NOT NULL THEN json_build_object(
    ...
    `;
      ...
      const stockLots = (p.stock_lots || []).map((lot) => ({
        lot_id: lot.lot_id,
        qty_in: Number(lot.qty_in || 0),
        qty_remaining: Number(lot.qty_remaining || 0),
        cp: lot.cp !== null ? Number(lot.cp) : 0,
        sp: lot.sp !== null ? Number(lot.sp) : 0,
        notes: lot.notes,
      ...
      const stock = stockLots.reduce(
        (total, lot) => total + Number(lot.qty_remaining || 0),
        0,
      );
