        ...
        for (const cat of categories) {
          try {
            ...
            if (operation === 'create') {
              ...
              const updateResult = await this.handleUpdate(owner_id, 'category', cat);
              result.synced.categories.push(updateResult);
            } else if (operation === 'delete') {
              const deleteResult = await this.handleDelete(owner_id, 'category', cat);
              result.synced.categories.push(deleteResult);
            }
          } catch (err) {
            result.failed.push({
              type: "category",
        ...
        for (const brand of brands) {
          try {
            ...
            if (operation === 'create') {
              ...
              const updateResult = await this.handleUpdate(owner_id, 'brand', brand);
              result.synced.brands.push(updateResult);
            } else if (operation === 'delete') {
              const deleteResult = await this.handleDelete(owner_id, 'brand', brand);
              result.synced.brands.push(deleteResult);
            }
          } catch (err) {
            result.failed.push({
              type: "brand",
        ...
        for (const unit of units) {
          try {
            ...
            if (operation === 'create') {
              ...
              const updateResult = await this.handleUpdate(owner_id, 'unit', unit);
              result.synced.units.push(updateResult);
            } else if (operation === 'delete') {
              const deleteResult = await this.handleDelete(owner_id, 'unit', unit);
              result.synced.units.push(deleteResult);
            }
          } catch (err) {
            result.failed.push({
              type: "unit",
        ...
        for (const supplier of suppliers) {
          try {
            ...
            if (operation === 'create') {
              ...
              const updateResult = await this.handleUpdate(owner_id, 'supplier', supplier);
              result.synced.suppliers.push(updateResult);
            } else if (operation === 'delete') {
              const deleteResult = await this.handleDelete(owner_id, 'supplier', supplier);
              result.synced.suppliers.push(deleteResult);
            }
          } catch (err) {
            result.failed.push({
              type: "supplier",
        ...
        for (const product of products) {
          try {
            ...
            if (operation === 'create') {
              ...
              const updateResult = await this.handleUpdate(owner_id, 'product', product);
              result.synced.products.push(updateResult);
            } else if (operation === 'delete') {
              const deleteResult = await this.handleDelete(owner_id, 'product', product);
              result.synced.products.push(deleteResult);
            }
          } catch (err) {
            result.failed.push({
              type: "product",
        ...
        for (const lot of stock_lots) {
          try {
            ...
            if (operation === 'create') {
              ...
              const updateResult = await this.handleUpdate(owner_id, 'stock_lot', lot);
              result.synced.stock_lots.push(updateResult);
            } else if (operation === 'delete') {
              const deleteResult = await this.handleDelete(owner_id, 'stock_lot', lot);
              result.synced.stock_lots.push(deleteResult);
            }
          } catch (err) {
            result.failed.push({
              type: "stock_lot",
        ...
        for (const sale of sales) {
          try {
            ...
            if (operation === 'create') {
              ...
              const updateResult = await this.handleUpdate(owner_id, 'sale', sale);
              result.synced.sales.push(updateResult);
            } else if (operation === 'delete') {
              const deleteResult = await this.handleDelete(owner_id, 'sale', sale);
        ...
        for (const ret of returns) {
          try {
            ...
            if (operation === 'create') {
              ...
              const updateResult = await this.handleUpdate(owner_id, 'return', ret);
              result.synced.returns.push(updateResult);
            } else if (operation === 'delete') {
              const deleteResult = await this.handleDelete(owner_id, 'return', ret);
