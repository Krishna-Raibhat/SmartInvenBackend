...
class ClothingNotificationPreferenceService {
  /**
  ...
  async getPreference(owner_id, type) {
    const lastPref = await prisma.clothingNotification.findFirst({
      where: {
        owner_id,
        type,
      },
  ...
  async setPreference(owner_id, type, is_enabled) {
    ...
    const preference = await prisma.clothingNotification.create({
      data: {
        owner_id,
        type,
...
export default new ClothingNotificationPreferenceService();
