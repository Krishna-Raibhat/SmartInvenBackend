...
class GrocerySupplierReturnService {
  /**
  ...
  async createReturn(owner_id, payload) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        const qtyRemaining = parseFloat(lot.qty_remaining.toString());
        if (qtyRemaining < qty) {
          const e = new Error(
      ...
      const returnHeader = await tx.grocerySupplierReturn.create({
        data: {
          owner_id,
          supplier_id,
      ...
      for (const item of returnItems) {
        // Create return item
        await tx.grocerySupplierReturnItem.create({
          data: {
      ...
      return tx.grocerySupplierReturn.findUnique({
   ...
   */
  async list(owner_id) {
    return prisma.grocerySupplierReturn.findMany({
      where: { owner_id },
  ...
  async getById(owner_id, return_id) {
    return prisma.grocerySupplierReturn.findFirst({
...
export default new GrocerySupplierReturnService();
